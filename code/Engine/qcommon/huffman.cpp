...
//  Description:  This is based on the Adaptive Huffman algorithm described
//                in Sayood's Data Compression book. The ranks are not
//                actually stored, but implicitly defined by the location
...
void Huff_Decompress( msg_s* mbuf, int offset )
{
	int         ch, cch, i, j, size;
	byte        seq[65536];
	byte*       buffer;
...
void Huff_Compress( msg_s* mbuf, int offset )
{
	int         i, ch, size;
	byte        seq[65536];
	byte*       buffer;
