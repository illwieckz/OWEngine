...
#else
	g_qeglobals.d_hInstance = GetModuleHandle( 0 );
	MFCCreate( GetModuleHandle( 0 );
#endif
...
BOOL CMainFrame::OnCreateClient( LPCREATESTRUCT lpcs, CCreateContext* pContext )
{
	...
	*/
	...
	g_PrefsDlg.m_nView = 0;
	if ( g_PrefsDlg.m_nView == 0 || g_PrefsDlg.m_nView == 3 )
	{
		// http://msdn.microsoft.com/en-us/library/87z44b55.aspx
		// returns non zero if success
		bool result_wndSplit =   m_wndSplit.CreateStatic( this, 2, 1 );
		bool result_wndSplit2 =  m_wndSplit2.CreateStatic( &m_wndSplit, 1, 3 );
		bool result_wndSplit3 =  m_wndSplit3.CreateStatic( &m_wndSplit2, 2, 1 );
		
		CRuntimeClass* editWndClass = RUNTIME_CLASS( CEditWnd );
		bool resultView_wndSplit =  m_wndSplit.CreateView( 1, 0, editWndClass, CSize( 25, 100 ), pContext );
		g_pEdit = dynamic_cast<CEdit*>( m_wndSplit.GetPane( 1, 0 ) );
		if ( g_pEdit )
			g_qeglobals.d_hwndEdit = g_pEdit->GetSafeHwnd();
			
		CRuntimeClass* camWndClass = RUNTIME_CLASS( CCamWnd );
		bool resultView_wndSplit3 = m_wndSplit3.CreateView( 0, 0, camWndClass, CSize( 25, 100 ), pContext );
	...
	else if ( g_PrefsDlg.m_nView == 1 )
	{
		...
		bool camWndCreateResult =  m_pCamWnd->Create( CAMERA_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1234 );
		if ( !camWndCreateResult )
			return true;
		m_pZWnd = new CZWnd();
		bool zWndCreateResult =  m_pZWnd->Create( Z_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1238 );
		if ( !zWndCreateResult )
			return true;
